...
//  1. _verifyAnswer passes registeredFacilityId to fetchDemographics so the
//     gateway queries the patient's HOME facility, not the requesting one.
//  2. fetchDemographics in HieApiService must accept an optional facilityId
//     param and append ?facility=ID to the request (see comment at bottom).
//  3. After verification, encounters are fetched via fetchEverything which also
//     passes ?registeredFacility=ID so home-facility encounters are included.
...
class _PatientLookupPageState extends State<PatientLookupPage> {
  ...
  Future<void> _fetchQuestion() async {
    ...
    try {
      ...
      if (result.success && result.question != null) {
        setState(() { _securityQuestion = result.question; _step = 1; });
      } else {
  ...
  // ── Step 1: verify answer → demographics + encounters ────────────────────
  Future<void> _verifyAnswer() async {
    final answer = _answerController.text.trim();
    if (answer.isEmpty) { _setError('Please enter your answer'); return; }
    final authState = context.read<AuthBloc>().state;
    if (authState is! Authenticated) return;
    setState(() { _loading = true; _error = null; });
    ...
    try {
      ...
      if (!result.success) { _setError(result.error ?? 'Incorrect answer'); return; }
      ...
      // ✅ FIX: get registered facility from verify response.
      //        This is the facility that holds the patient's FHIR record.
      ...
      // ── Fetch demographics from the REGISTERED facility ──────────────────
      // ✅ FIX: pass registeredFacilityId so gateway appends ?facility=ID
      ...
      // ── Fetch cross-facility encounters ──────────────────────────────────
      // ✅ NEW: always include registered facility via ?registeredFacility=
      List<Map<String, dynamic>> crossEncounters = [];
      try {
        ...
        if (encResult.success && encResult.data != null) {
          final entries = encResult.data!['entry'] as List? ?? [];
          crossEncounters = entries
        ...
        debugPrint('Could not fetch encounters: $encErr');
      }
      ...
      final demographics = <String, dynamic>{
        ...
        'registeredFacility':   patientMeta['registeredFacility']   ?? demoResult.data?['registeredFacility'] ?? '',
        ...
        '_accessToken':         token,
        '_encounters':          crossEncounters,
      };

      setState(() { _patientData = demographics; _step = 2; });
    } catch (e) {
      _setError('Network error: ${e.toString().split('\n').first}');
    } finally {
      if (mounted) setState(() => _loading = false);
  ...
  void _setError(String msg) { if (mounted) setState(() => _error = msg); }

  void _reset() {
    setState(() {
      _step = 0; _error = null;
      _securityQuestion = null; _patientData = null;
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => CreateEncounterPage(
          patient:     Patient.fromHieData(data: data, facilityId: authState.user.facilityId),
          nupiPatient: data,
        ),
      ),
    );
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_error != null) ...[_buildErrorBanner(_error!), const SizedBox(height: 16)],
            AnimatedSwitcher(
  ...
  Widget _buildStepIndicator() {
    ...
    return Row(children: List.generate(labels.length * 2 - 1, (i) {
      if (i.isOdd) {
        return Expanded(child: Container(height: 2, margin: const EdgeInsets.only(bottom: 16),
            color: _step > i ~/ 2 ? _primary : Colors.grey[200]));
      }
      final idx = i ~/ 2; final done = _step > idx; final active = _step == idx;
      return Column(children: [
        AnimatedContainer(
          duration: const Duration(milliseconds: 300),
          width: 28, height: 28,
          decoration: BoxDecoration(color: done || active ? _primary : Colors.grey[200],
              shape: BoxShape.circle),
          child: Center(child: done
              ? const Icon(Icons.check, color: Colors.white, size: 14)
              : Text('${idx + 1}', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold,
  ...
  Widget _buildStep0({Key? key}) => Column(
    ...
    children: [
      ...
      _label('National ID / Passport Number'), const SizedBox(height: 8),
      _inputField(controller: _nationalIdController, hint: 'e.g. 12345678',
          icon: Icons.badge_outlined, inputType: TextInputType.number),
      const SizedBox(height: 16),
      _label('Date of Birth'), const SizedBox(height: 8),
      GestureDetector(onTap: _pickDob, child: AbsorbPointer(child:
          _inputField(controller: _dobController, hint: 'Select date of birth', icon: Icons.cake_outlined))),
      const SizedBox(height: 28),
      _primaryButton(label: 'Find Patient', icon: Icons.search_rounded, loading: _loading, onTap: _fetchQuestion),
    ],
  );
  ...
  Widget _buildStep1({Key? key}) => Column(
    ...
    children: [
      Container(
        ...
        decoration: BoxDecoration(color: _primary.withOpacity(0.05),
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Security Question', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                color: Color(0xFF94A3B8), letterSpacing: 0.3)),
            const SizedBox(height: 4),
            Text(_securityQuestion ?? '', style: const TextStyle(fontSize: 14,
                fontWeight: FontWeight.w700, color: Color(0xFF0F172A))),
          ])),
        ]),
      ),
      const SizedBox(height: 20),
      _label('Your Answer'), const SizedBox(height: 8),
      _inputField(controller: _answerController, hint: 'Type your answer', icon: Icons.vpn_key_outlined),
      const SizedBox(height: 28),
      _primaryButton(label: 'Verify Identity', icon: Icons.verified_user_outlined,
          loading: _loading, onTap: _verifyAnswer),
      const SizedBox(height: 12),
      Center(child: TextButton.icon(onPressed: _reset,
          icon: const Icon(Icons.arrow_back, size: 16), label: const Text('Start Over'),
  ...
  Widget _buildStep2({Key? key}) {
    ...
    final county            = data['county']?.toString()
                           ?? (data['address'] as Map?)?['district']?.toString() ?? '';
    final subCounty         = data['subCounty']?.toString()
                           ?? (data['address'] as Map?)?['city']?.toString()     ?? '';
    ...
    final encounters        = (data['_encounters'] as List?)
                                  ?.cast<Map<String, dynamic>>() ?? [];
      ...
      children: [
        Row(children: [
          Container(
            ...
            decoration: BoxDecoration(color: const Color(0xFF2D6A4F).withOpacity(0.1),
                borderRadius: BorderRadius.circular(20)),
            child: const Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.check_circle_rounded, color: Color(0xFF2D6A4F), size: 14),
              SizedBox(width: 6),
              Text('Patient Verified', style: TextStyle(fontSize: 12,
                  fontWeight: FontWeight.w700, color: Color(0xFF2D6A4F))),
            ]),
          ),
        ]),
        const SizedBox(height: 16),
        Container(
          ...
          decoration: BoxDecoration(color: Colors.white,
              borderRadius: BorderRadius.circular(20),
              border: Border.all(color: const Color(0xFF2D6A4F).withOpacity(0.25), width: 2)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(width: 60, height: 60,
                decoration: BoxDecoration(shape: BoxShape.circle,
                    border: Border.all(color: genderColor, width: 3)),
                child: CircleAvatar(backgroundColor: _primary.withOpacity(0.08),
            ...
            const SizedBox(height: 16),
            _sectionLabel('Demographics'), const SizedBox(height: 12),
            ...
            if (subCounty.isNotEmpty) _detailRow(Icons.map_outlined, 'Sub-County', subCounty),
            const SizedBox(height: 16),
            const Divider(color: Color(0xFFF1F5F9)),
            const SizedBox(height: 16),
            _sectionLabel('Registered Facility'), const SizedBox(height: 12),
            Row(children: [
              Container(padding: const EdgeInsets.all(10),
                decoration: BoxDecoration(
                  color: (isCurrentFacility ? const Color(0xFF2D6A4F) : const Color(0xFF0EA5E9)).withOpacity(0.1),
                  borderRadius: BorderRadius.circular(12)),
                child: Icon(Icons.local_hospital_rounded,
                  color: isCurrentFacility ? const Color(0xFF2D6A4F) : const Color(0xFF0EA5E9), size: 20)),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(facilityName, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14)),
            ...
            // ✅ Encounter history from registered facility
            if (encounters.isNotEmpty) ...[
              ...
              ...encounters.take(5).map((enc) {
                final encType  = enc['class']?['display']?.toString()
                              ?? enc['type']?[0]?['text']?.toString() ?? 'Visit';
                final encDate  = enc['period']?['start']?.toString() ?? '';
                final facName  = enc['meta']?['sourceName']?.toString() ?? facilityName;
                return Padding(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(encType, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                      Text('$facName${encDate.isNotEmpty ? " · $encDate" : ""}',
  ...
  Widget _infoBanner(IconData icon, String title, String sub) => Container(
    ...
    decoration: BoxDecoration(color: _primary.withOpacity(0.05),
        ...
        border: Border.all(color: _primary.withOpacity(0.15))),
    child: Row(children: [
      Icon(icon, color: _primary, size: 26), const SizedBox(width: 12),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(title, style: const TextStyle(fontWeight: FontWeight.w800, color: _primary, fontSize: 14)),
        const SizedBox(height: 2),
        Text(sub, style: const TextStyle(fontSize: 12, color: Color(0xFF374151), height: 1.4)),
      ])),
    ]),
  );
  ...
  Widget _detailRow(IconData icon, String label, String value) {
    ...
    return Padding(padding: const EdgeInsets.only(bottom: 10),
      child: Row(children: [
        Icon(icon, size: 16, color: Colors.grey[400]), const SizedBox(width: 10),
