...
//
// Flow:
//   Step 0 — Enter National ID + DOB
//             → GET /api/patients/verify/question?nationalId=X&dob=Y
//             → shows the patient's security question
//   Step 1 — Answer security question
//             → POST /api/patients/verify/answer { nationalId, dob, answer }
//             → shows full demographics card + "Create Encounter" button
// ─────────────────────────────────────────────────────────────────────────────
...
class _PatientLookupPageState extends State<PatientLookupPage> {
  ...
  // ── State ─────────────────────────────────────────────────────────
  int     _step              = 0; // 0 = ID+DOB, 1 = security question, 2 = result
  bool    _loading           = false;
  String? _error;
  String? _securityQuestion;
  Map<String, dynamic>? _patientData; // full demographics from /verify/answer
  ...
  void _createEncounter(BuildContext context) {
    ...
    // Build a minimal Patient from HIE demographics so CreateEncounterPage
    // can auto-persist it to Firestore if not already present.
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => CreateEncounterPage(
          patient:    hiPatient,
          nupiPatient: data, // raw HIE map — bloc will upsert if needed
        ),
      ),
    );
  }
  ...
  // ─────────────────────────────────────────────────────────────────
  // Step 2 — Full Demographics Result
  // ─────────────────────────────────────────────────────────────────
